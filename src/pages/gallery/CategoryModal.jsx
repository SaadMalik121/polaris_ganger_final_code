...
export default function CategoryModal({
  ...
  const isDisabledEditButton = () => {
    const { title, active } = savedCategory;
    const isActive = {
      0: false,
      1: true,
    };
    if (
      ...
      isActive[active] === selectedActiveStatus
    ) {
  ...
  useEffect(() => {
    if (savedCategory) isDisabledEditButton();
  }, [savedCategory, newCategoryTitle, selectedActiveStatus]);
      ...
      <Modal
        ...
        primaryAction={{
          content: (
            <FormattedMessage
              id={
                editCategoryData ? "editCategorySaveBtn" : "addCategoryModalBtn"
              }
            />
          ),
          loading: loadingAddBtn,
          onAction: handleSaveCategory,
          disabled: savedCategory ? isEditDisabled : false,
        }}
        secondaryActions={
          editCategoryData && {
